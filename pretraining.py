import torch
#from EPMolGen.gdbp_model import epmolgen, epmolgenWithEdgeNew
from EPMolGen.EPMolGen_model import epmolgen
#from EPMolGen.utils import Experiment, LoadDataset
from EPMolGen.tools.train import Experiment
...
encoder_cfg = EasyDict(
     ...
     'knn':16, 'cutoff':10.0, 'no_protein':True}
     )
...
config = EasyDict(
    {'deq_coeff':0.9, 'hidden_channels':64, 'hidden_channels_vec':16, 'use_conv1d':False, 'num_bond_types':4,
     ...
     'edge_flow':edge_flow_cfg, 'pivotal_net':pivotal_net_cfg, 'pos_filter':pos_filter_cfg}
    )
...
pivotal_masker = PivotalMasker(r=6.0, num_work=16, atomic_numbers=[6,7,8,9,15,16,17,35,53])
graph_compose = GraphCompose(
    knn=16, num_workers=16, graph_type='knn', radius=10.0, use_protein_bond=False
    )
combine = Combine(traj_fn, pivotal_masker, graph_compose, lig_only=True)
...
dataset = LoadDataset('/DATA/yzy/EPMolGen_pretraining_dataset/pretraining_dataset.lmdb',transform = transform)
print('Num data:', len(dataset))
train_set, valid_set = LoadDataset.split(dataset, val_num=10000, shuffle=True, random_seed=0)
device = 'cuda:0'
model = epmolgen(config).to('cuda:0')
#model.load_state_dict(ckpt['model'])



#model = epmolgenWithEdgeNew(config).to('cuda:0')
...
exp = Experiment(
    ...
    device='cuda:0', use_amp=False
    )

"""exp.fit_step(
    1000000, valid_per_step=5000, train_batch_size=128, valid_batch_size=16, print_log=False,
...
exp.fit_step(
    1000000, valid_per_step=5000, train_batch_size=16, valid_batch_size=16, print_log=False,
    with_tb=True, logdir='./pretraining_log', schedule_key='loss', num_workers=16, 
    pin_memory=False, follow_batch=[], exclude_keys=[], collate_fn=None, 
    max_edge_num_in_batch=2000000
    )