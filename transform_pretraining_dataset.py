...
import torch
from EPMolGen.tools import Protein, Ligand, ComplexData, torchify_dict, is_in_ring
import numpy as np
...
empty_pocket_dict.atom_partial_charge = np.empty(0, dtype=np.float32)
empty_pocket_dict.atom_formal_charge = np.empty(0, dtype=np.float32)
...
sdf_path='./pretraining_dataset/*/*'
processed_path='./pretraining_dataset.lmdb'
sdf_list = glob.glob(sdf_path+'/*.gz')
db = lmdb.open(
            processed_path,
