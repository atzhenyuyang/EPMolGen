...
def sample_atom_num(atom_num_scope, distribution='normal'):
    ...
    if distribution == 'normal':
        ...
        std = 1
        num_atom_gen = np.random.normal(mean, std, 1).astype(np.int64).clip(min_atom_num, max_atom_num).item()
        return num_atom_gen
    else:
...
class Generate(object):
    def __init__(self, model, transform, temperature=[1.0, 1.0], atom_type_map=[6,7,8,9,15,16,17,35,53],
                 num_bond_type=4, max_atom_num=35, pivotal_threshold=0.5, max_double_in_6ring=0, min_dist_inter_mol=3.0, bond_length_range=(1.0, 2.0),
    ...
    @staticmethod
    def __choose_focal(focal_net, h_ctx, ctx_idx, pivotal_threshold, choose_max, surf_mask=None):
        ...
        if choose_max:
            ...
            if isinstance(surf_mask, torch.Tensor) and surf_mask.sum() > 0:
                ...
                surf_focal_mask = focal_prob_surf > pivotal_threshold
                ...
                focal_mask = (focal_prob >= pivotal_threshold).view(-1)
    ...
    def atom_generate(self, focal_idx, focal_prob, atom_idx, compose_feature_sca_transform,
    ...
    def run(self, data):
        ...
        with torch.no_grad():
            ...
            for atom_idx in range(begin_atom_idx, self.max_atom_num):
                data = data.to(self.device)
                h_cpx = embed_compose(
                ...
                # get position of new atom 
                    ...
                    sca_focal, vec_focal = h_cpx[0][focal_idx], h_cpx[1][focal_idx]
                    ...
                    new_atom_type, focal_idx = self.atom_generate(
                        focal_idx, focal_prob, atom_idx, compose_feature_sca_transform,
                        compose_feature_vec_transform
                        )
                ...
                if self.resample_edge_faild or self.resample_node > self.max_resample_node or focal_out is False:
                    break
                else:
                    data = data.to('cpu')
    ...
    def generate(self, data, num_gen=100, rec_name='recptor', with_print=True, root_path='gen_results'):
        ...
        for i in range(num_gen):
            self.max_atom_num = sample_atom_num([10,28], distribution='normal')
