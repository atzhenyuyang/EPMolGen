...
from .tools import get_tri_edges
from .tools import add_ligand_atom_to_data, data2mol, modify, check_valency, check_alert_structures
from .EPMolGen_model import embed_compose
from .tools import verify_dir_exists, substructure
...
def sample_atom_num(atom_num_scope, distribution='normal'):
    ...
    if distribution == 'normal':
        ...
        std = 1
        num_atom_gen = np.random.normal(mean, std, 1).astype(np.int64).clip(min_atom_num, num_atom_max).item()
        return num_atom_gen
    else:
...
class Generate(object):
    def __init__(self, model, transform, temperature=[1.0, 1.0], atom_type_map=[6,7,8,9,15,16,17,35,53],
                 num_bond_type=4, num_atom_max=35, pivotal_threshold=0.5, max_double_in_6ring=0, min_dist_inter_mol=3.0, bond_length_range=(1.0, 2.0),
                 choose_max=True, device='cuda:0',max_resample_node=100, max_resample_edge=100):
        ...
        self.num_atom_max = num_atom_max
    ...
    @staticmethod
    def __choose_pivotal(pivotal_net, h_ctx, ctx_idx, pivotal_threshold, choose_max, surf_mask=None):
    ...
    def choose_pivotal(self, h_cpx, cpx_index, idx_ligand_ctx_in_cpx, data, atom_idx):
        if atom_idx == 0:
            ...
            pivotal_idx_, pivotal_prob = self.__choose_pivotal(
                ...
                self.pivotal_threshold, self.choose_max, #1
                surf_mask=surf_mask)
        else:
            pivotal_idx_, pivotal_prob = self.__choose_pivotal(
                self.model.pivotal_net, h_cpx, idx_ligand_ctx_in_cpx, 
                self.pivotal_threshold, 1#self.choose_max
                )
        if pivotal_idx_ is False:
            return False
        
        # valence_check for pivotal atom
    ...
    def atom_generate(self, pivotal_idx, pivotal_prob, atom_idx, compose_feature_sca_transform,
            compose_feature_vec_transform):  # 生成新原子的类型
        latent_atom = self.prior_node.sample([pivotal_idx.size(0)])
        latent_atom = self.model.atom_flow.reverse(latent_atom, compose_feature_sca_transform,
    ...
    def pos_generate(self, h_cpx, atom_type_emb, pivotal_idx, cpx_pos, atom_idx):
        ...
        if atom_idx != 0:
            dist_mask = (dist>self.bond_length_range[0]) == (dist<self.bond_length_range[1])  # check if it matches chemical rules
    ...
    def run(self, data):
        ...
        with torch.no_grad():
            ...
            self.counter = 0
            #new_pos_traj = torch.empty(1,3).to('cuda:0')
            for atom_idx in range(begin_atom_idx, self.num_atom_max):
                data = data.to(self.device)
                h_cpx = embed_compose(
                ...
                h_cpx = self.model.encoder(
                    ...
                    annealing=self.model.msg_annealing
                )
                ...
                # get position of new atom 
                    ...
                    if new_pos_to_add is False:
                        ...
                        continue
                    data.step_batch = torch.zeros(data.cpx_pos.size(0)).to('cuda:0')    
                    #edge_index_0, edge_index_1 = knn(x= data.cpx_pos, y= new_pos_traj, k=32, batch_x = data.step_batch, batch_y = torch.arange(0, new_pos_traj.size()[0]).to('cuda:0'))
                    edge_index_0, edge_index_1 = knn(x= data.cpx_pos, y= new_pos_to_add, k=32, batch_x = data.step_batch, batch_y = torch.arange(0, new_pos_to_add.size()[0]).to('cuda:0'))
                    sca_pos, vec_pos = elect_field(data.cpx_pos[edge_index_1], data.cpx_charge[edge_index_1], new_pos_to_add[edge_index_0])
                    sca_ele = scatter_add(sca_pos, edge_index_0)
                    vec_ele = scatter_add(vec_pos, edge_index_0, dim = 0)
                    #pos_sca, pos_vec = sca_ele.unsqueeze(-1), vec_ele.unsqueeze(1)                       
                    vec_ele = vec_ele.unsqueeze(1)
                    sca, vec = self.model.electric_transform(sca_ele, vec_ele)                                 
                    #h_cpx[0][edge_index_1].shape => torch.Size([768, 64]);  h_cpx[1][edge_index_1].shape torch.Size([768, 16, 3])
                    ...
                    sca_pivotal, vec_pivotal = h_cpx[0][pivotal_idx], h_cpx[1][pivotal_idx]
                    sca = torch.cat((sca_pivotal, compose_feature_sca), dim = 1)
                    vec = torch.cat((vec_pivotal, compose_feature_vec), dim = 1)
                    compose_feature_sca_transform = self.model.dimension_transform_sca(sca)
                    compose_feature_vec_transform = self.model.dimension_transform_vec(vec)
                    
                    
                    # generate atom # 生成新原子的元素类型
                    new_atom_type, pivotal_idx = self.atom_generate(
                        pivotal_idx, pivotal_prob, atom_idx, compose_feature_sca_transform,
                        compose_feature_vec_transform
                        )
                    ...
                    if bond_out is not False:
                        ...
                        has_alert = check_alert_structures(rw_mol, ['[O]-[O]','[N]-[O,Br,Cl,I,F,P]','[S,P]-[Br,Cl,I,F]','[P]-[O]-[P]','[Br,Cl,I,F]-[Br,Cl,I,F]', '[O]-[O,Br,Cl,I,F]','[N,P]-[Br,Cl,I,F,P]','[S,P]-[Br,Cl,I,F]','[P]-[O]-[P]', '[C]-[S]=[C]',
                                                                    '[Br,Cl,I,F]-[Br,Cl,I,F]','[C,N,O,S]=[C]=[C,N,O,S]','[C,N,S,I]=[P]','[I]=[N,C,O]', '[C]-[I]-[C]'])
                        if has_alert:
                            for ix in range(new_edge_idx.size(1)):
                                i, j = new_edge_idx[:, ix].tolist()
                                rw_mol.RemoveBond(atom_idx, j)
                ...
                if self.resample_edge_faild or self.resample_node > self.max_resample_node or pivotal_out is False:
                    ...
                    data = data.to('cpu')
                    data, rw_mol_ = add_ligand_atom_to_data(
                        data, 
    ...
    def generate(self, data, molecule_number=100, rec_name='recptor', print_SMILES=True, root_path='gen_results'):
        ...
        for i in range(molecule_number):
            self.num_atom_max = sample_atom_num([10,28], distribution='normal')
        ...
        print('Validity: {:.4f}'.format(len(smiles_list)/molecule_number))
        print('Unique: {:.4f}'.format(len(set(smiles_list))/len(smiles_list)))
        out_statistic = {
            'Validity':len(smiles_list)/molecule_number,
