...
from .protein_file_transformer import Protein, parse_sdf_to_dict, Ligand
from .process_residues import RESIDUES_TOPO
...
class SplitPocket(object):
    ...
    def _split_pocket_with_surface_atoms(protein, ligand, dist_cutoff):
        ...
        #sequence, res_idx_map = protein.sequence("A")
        #sequence, res_idx_map = protein.sequence()

        topo = protein.get_topo()
        pocket_res_topo = {}
        pocket_block = []
        for r in pocket_res:
            pocket_res_topo[r.idx] = topo[r.idx]
            for a in r.get_atoms:
                if a.element == 'H':
                    pocket_block.append(a.to_string + ' ')
                elif a.is_surf is True:
                    pocket_block.append(a.to_string + ' surf')
                else:
                    pocket_block.append(a.to_string + ' inner')
        #seq_info = f'RESSEQ {sequence}\nRESMAP {str(res_idx_map)}\n'
        #pocket_block = seq_info + '\n'.join(pocket_block)
        pocket_block = '\n'.join(pocket_block)
        
        #return pocket_block, ligand.mol_block(), pocket_res_topo
        #return pocket_block
        return pocket_res_topo, pocket_block
