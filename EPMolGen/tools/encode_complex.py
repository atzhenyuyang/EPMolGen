...
class ComplexData(Data):
    ...
    def __inc__(self, key, value, *args, **kwargs):
        KEYS = {
            'idx_ligand_ctx_in_cpx', 'idx_protein_in_cpx', 'pivotal_idx_in_cpx', 'pivotal_idx_in_context_',
            'pivotal_idx_in_context', 'cpx_knn_edge_index', 'edge_query_index_1', 'pos_query_edge_idx_1',
            'pos_fake_knn_edge_idx_1', 'pos_real_knn_edge_idx_1', 'tri_edge_index', 'apo_protein_idx',
            'candidate_pivotal_idx_in_protein', 'cpx_backbone_index'
            }
        if key in KEYS:
        ...
        '''if key == 'idx_ligand_ctx_in_cpx':
            ...
            return self['cpx_pos'].size(0)
        elif key == 'pivotal_idx_in_cpx':
            return self['cpx_pos'].size(0)
        elif key == 'pivotal_idx_in_context_':
            return self['cpx_pos'].size(0)
        elif key == 'pivotal_idx_in_context':
            return self['cpx_pos'].size(0)
            ...
            return self['cpx_pos'].size(0)
        elif key == 'candidate_pivotal_idx_in_protein':
            return self['cpx_pos'].size(0)
        elif key == 'step_batch':
            return self['step_batch'].max() + 1
        elif key == 'ligand_bond_index':
