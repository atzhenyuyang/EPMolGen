...
def add_ligand_atom_to_data(
    ...
    modified_2 = False
    if len(bond_type) != 0:
        bond_index, bond_type, modified_1 = remove_triangle(
        ...
        bond_type, modified_2 = check_valence_is_2(
                            bond_index, bond_type,
    ...
    if modified_1 + modified_2:
        ...
        for b_ix, b in enumerate(data.ligand_context_bond_index.T):
            ...
            if b1 in l_ or b2 in l_:
                ...
                rw_mol.AddBond(b1[0], b1[1], bond_type_map[bond_type])
                l_.append(b1)
                l_.append(b2)
        return data, rw_mol
    else:
...
def check_valence_is_2(
    ...
    if new_atom_type_is_C:
        ...
        bond_type_to_add = bond_type_to_add - mask.long()
        modified = True
    return bond_type_to_add, modified
...
def remove_triangle(pos_to_add, ligand_context_pos, ligand_context_bond_index, ligand_context_bond_type, 
    ...
    if adj_mask.sum() > 0:
        ...
        modified = True
    return bond_index_to_add, bond_type_to_add, modified
