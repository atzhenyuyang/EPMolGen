...
from .encode_complex import ComplexData
...
def elect_field(pos, partial_charge, pos_ca):  
    ...
    vec = field_vec / dist.unsqueeze(-1) 
...
class PivotalMasker(object):     
    ...
    def run(self, data):
        if data.ligand_context_pos.size(0) == 0:
            ...
            pivotal_idx_in_context = torch.norm(data.protein_pos-masked_pos.unsqueeze(0), p=2, dim=-1).argmin()
            data.pivotal_idx_in_context = pivotal_idx_in_context.unsqueeze(0)
            data.pivotal_idx_in_context_ = pivotal_idx_in_context.unsqueeze(0)
            ...
            data.pivotal_label = torch.empty(0, dtype=torch.long)
            ...
            # candidate pivotal atom without ligand
            ...
            data.candidate_pivotal_idx_in_protein = torch.unique(assign_index[0])
            candidate_pivotal_label_in_protein = torch.zeros(data.protein_pos.size(0), dtype=torch.bool)
            candidate_pivotal_label_in_protein[data.candidate_pivotal_idx_in_protein] = True
            data.candidate_pivotal_label_in_protein = candidate_pivotal_label_in_protein
            data.apo_protein_idx = torch.arange(data.protein_pos.size(0), dtype=torch.long)
        else:
            new_step_atom_idx = data.masked_idx[0]
            candidate_pivotal_idx_in_context = torch.LongTensor(data.ligand_nbh_list[new_step_atom_idx.item()])
            pivotal_idx_in_context_mask = (data.context_idx.unsqueeze(1) == candidate_pivotal_idx_in_context).any(1)
            data.pivotal_idx_in_context_ = torch.nonzero(pivotal_idx_in_context_mask).view(-1)
            #data.idx_pivotal_in_protein = torch.empty(0, dtype=torch.long)
            pivotal_choice_idx = torch.multinomial(torch.ones_like(data.pivotal_idx_in_context_).float(), 1)
            data.pivotal_idx_in_context = data.pivotal_idx_in_context_[pivotal_choice_idx]
            data.pivotal_label = torch.zeros_like(data.context_idx)
            data.pivotal_label[data.pivotal_idx_in_context] = 1
            data.atom_label = torch.nonzero(data.ligand_element[data.masked_idx[0]] == self.atomic_numbers).squeeze(0)
            data.apo_protein_idx = torch.empty(0, dtype=torch.long)
            data.candidate_pivotal_idx_in_protein = torch.empty(0, dtype=torch.long)
            data.candidate_pivotal_label_in_protein = torch.empty(0, dtype=torch.long)
            #pivotal_idx_in_ligand = data.context_idx[pivotal_idx_in_context_mask]
            # get edge label
...
class GraphCompose(object):
    ...
    def run(self, data):
        ...
        if data.idx_ligand_ctx_in_cpx.size(0) == 0:
            data.cpx_charge= data.protein_atom_partial_charge    #append(torch.cat([torch.zeros((d.idx_ligand_ctx_in_cpx.size(0))),d.protein_atom_charge], dim = 0))
            data.cpx_charge_pos= data.protein_pos #append(d.cpx_pos)
        else:
            data.cpx_charge= torch.cat([torch.zeros(data.ligand_context_pos.size(0), dtype=torch.long), data.protein_atom_partial_charge], dim=0)    #append(torch.cat([torch.zeros((d.idx_ligand_ctx_in_cpx.size(0))),d.protein_atom_charge], dim = 0))
            data.cpx_charge_pos= torch.cat([data.ligand_context_pos,data.protein_pos], dim=0)#append(d.cpx_pos)        
...
class Combine(object):
    def __init__(self, lig_traj, pivotal_maker, graph_compose, lig_only=False):
        self.lig_traj =lig_traj
        self.pivotal_maker = pivotal_maker
        self.graph_compose = graph_compose
        ...
        for ix, i in enumerate(perm):
            ...
            if ix == 0:
                ...
                out = mask_node(data_step, perm[:ix], perm[ix:], 
                                ...
                                y_pos_std=self.lig_traj.y_pos_std)
                #out.edge_index_step = torch.LongTensor(edge_index[ix]).permute(-1,0)
            out = self.pivotal_maker.run(out)
            out = self.graph_compose.run(out)
            traj.append(out)
        del data
        return traj

COLLATE_KEYS = [
                'cpx_pos', 'cpx_feature', 'idx_ligand_ctx_in_cpx', 'idx_protein_in_cpx', 'pivotal_idx_in_context', 
                'pivotal_idx_in_context_', 'cpx_edge_index', 'cpx_edge_type', 'cpx_edge_feature',  'pos_query_knn_edge_idx_0',
                'pos_query_knn_edge_idx_1', 'pivotal_label', 'ligand_frontier', 'y_pos', 'edge_label', 'atom_label', 'edge_query_index_0', 
                ...
                'tri_edge_feat', 'cpx_pos_batch', 'y_pos_batch', 'edge_label_batch', 'atom_label_batch',
                'apo_protein_idx', 'candidate_pivotal_idx_in_protein', 'candidate_pivotal_label_in_protein', 'step_batch',
                'ligand_ctx_step', 'cpx_charge', 'cpx_charge_pos', 'protein_partial_charge'     
                ]
...
def collate_fn(data_list, collate_keys=COLLATE_KEYS):
    ...
    data_dict['protein_partial_charge'] = data_list[0].protein_atom_partial_charge
    ...
    for idx, d in enumerate(data_list):
        ...
        data_dict['candidate_pivotal_idx_in_protein'].append(d.candidate_pivotal_idx_in_protein)
        data_dict['candidate_pivotal_label_in_protein'].append(d.candidate_pivotal_label_in_protein)
        data_dict['apo_protein_idx'].append(d.apo_protein_idx)
        data_dict['pivotal_idx_in_context'].append(d.pivotal_idx_in_context+compose_pos_cusum)
        data_dict['pivotal_idx_in_context_'].append(d.pivotal_idx_in_context_+compose_pos_cusum)
        ...
        data_dict['cpx_charge'].append(torch.cat([torch.zeros((d.idx_ligand_ctx_in_cpx.size(0)), dtype = torch.long), d.protein_atom_partial_charge], dim = 0))   # 注意顺序
        data_dict['cpx_charge_pos'].append(d.cpx_pos)
        lig_cusum += d.idx_ligand_ctx_in_cpx.size(0)
        q_cusum += d.protein_atom_partial_charge.size(0)
        data_dict['pivotal_label'].append(d.pivotal_label)
    ...
    data_dict['candidate_pivotal_idx_in_protein'] = torch.cat(data_dict['candidate_pivotal_idx_in_protein'])
    data_dict['candidate_pivotal_label_in_protein'] = torch.cat(data_dict['candidate_pivotal_label_in_protein'])
    data_dict['pivotal_idx_in_context'] = torch.cat(data_dict['pivotal_idx_in_context'])
    data_dict['pivotal_idx_in_context_'] = torch.cat(data_dict['pivotal_idx_in_context_'])
    ...
    data_dict['cpx_charge'] = torch.cat(data_dict['cpx_charge'], dim=0)
    data_dict['cpx_charge_pos'] = torch.cat(data_dict['cpx_charge_pos'], dim=0)

    data_dict['pivotal_label'] = torch.cat(data_dict['pivotal_label'])
