...
from .module_blocks import GDBPerceptronVN, GDBLinear, MessageModule, MessageAttention, AttentionEdges, ST_GDBP_Exp, VNLeakyReLU
from .tool import GaussianSmearing, EdgeExpansion
import math
...
class PositionEncoder(Module):
    def __init__(self, in_sca, in_vec, edge_channels, num_filters, bottleneck=1, cutoff=10.,
        ...
        self.dimension_sca = nn.Sequential(
            nn.Linear(128, 64), 
            nn.LeakyReLU(), 
            nn.Linear(64, 64)
        )
        self.dimension_vec = nn.Sequential(
            VNLinear(32, 8, bias=False), 
            VNLeakyReLU(8), 
            VNLinear(8, 16, bias=False)
        )
    ...
    def forward(self, pos_query, edge_index_q_cps_knn, cpx_pos, node_attr_compose, atom_type_emb, 
        ...
        sca_feature = atom_type_emb
        vec_feature = root_vec_ij
        #sca_feature = self.dimension_sca(torch.cat((atom_type_emb, compose_feature_sca), dim=1))
        #vec_feature = self.dimension_vec(torch.cat((root_vec_ij, compose_feature_vec), dim=1))
        y_root_sca, y_root_vec = self.root_lin([sca_feature, vec_feature])
        #y_root_sca, y_root_vec = self.root_lin([atom_type_emb, root_vec_ij])  # torch.cat((atom_type_emb, compose_feature_sca), dim=1), torch.cat((root_vec_ij, compose_feature_vec), dim=1)
...
class BondFlow(Module):
    ...
    def forward(self, z_edge, pos_query, edge_index_query, cpx_pos, node_attr_compose, edge_index_q_cps_knn,
        ...
        """y[0] = self.dimension_sca(torch.cat((y[0],compose_feature_sca), dim = 1))
        y[1] = self.dimension_vec(torch.cat((y[1],compose_feature_vec), dim = 1))"""
