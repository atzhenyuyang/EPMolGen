...
class epmolgen(nn.Module):
    ...
    def get_loss(self, data):
        ...
        pivotal_loss = F.binary_cross_entropy_with_logits(
            input=pivotal_pred, target=data.ligand_frontier.view(-1, 1).float()
        )
        pivotal_pred_apo = self.pivotal_net(h_cpx, data.apo_protein_idx)
        surf_loss = F.binary_cross_entropy_with_logits(
            input=pivotal_pred_apo, target=data.candidate_pivotal_label_in_protein.view(-1, 1).float()
        )
        ...
        if pretraining == True:
            ...
            sca_pivotal, vec_pivotal = h_cpx[0][data.pivotal_idx_in_context], h_cpx[1][data.pivotal_idx_in_context]
